"""
LangChain integration for Bulwark Gateway SDK.
...
from bulwark_sdk.models import ScanResult, Verdict
...
class BulwarkCallbackHandler:
    """LangChain callback handler that enforces Bulwark security policies.

    Scans LLM prompts on `on_llm_start` and LLM responses on `on_llm_end`.
    Can operate in two modes:

    1. **Remote mode** — Uses a BulwarkClient to scan via the gateway API.
    2. **Local mode** — Uses a BulwarkGuard for offline regex-based scanning.

    Args:
        client: A BulwarkClient instance for remote scanning.
            Mutually exclusive with `guard`.
        guard: A BulwarkGuard instance for local scanning.
            Mutually exclusive with `client`. If neither is provided,
            a default BulwarkGuard is created.
    ...
    def _init_impl(
        ...
        **kwargs: Any,
        ...
        self._client = client
        self._guard = guard or (BulwarkGuard() if client is None else None)
        ...
        **kwargs: Any,
        ...
        for prompt in prompts:
            ...
            if result and result.is_blocked and self._block_on_detect:
                raise SecurityError(
                    f"Input blocked by Bulwark: {result.reason}",
                    result=result,
                )
            elif result and result.verdict == Verdict.WARN:
                logger.warning(
                    "bulwark_langchain_input_warning",
                    extra={
    ...
    def _on_llm_end_impl(self: Any, response: Any, **kwargs: Any) -> None:
        ...
        if text:
            ...
            if result and result.is_blocked and self._block_on_detect:
                raise SecurityError(
                    f"Output blocked by Bulwark: {result.reason}",
                    result=result,
                )
            elif result and result.verdict == Verdict.WARN:
                logger.warning(
                    "bulwark_langchain_output_warning",
                    extra={
                        "verdict": result.verdict.value,
                        "events_count": len(result.events),
                        "reason": result.reason,
        ...
        **kwargs: Any,
        ...
        for key in ("input", "query", "question", "prompt", "human_input"):
            if key in inputs and isinstance(inputs[key], str):
                ...
                if result and result.is_blocked and self._block_on_detect:
                    raise SecurityError(
                        f"Chain input blocked by Bulwark: {result.reason}",
                        result=result,
                    )
                break
    ...
    def _scan_content(self: Any, content: str, direction: str = "input") -> ScanResult | None:
        ...
        try:
            ...
            logger.warning(
                "bulwark_langchain_scan_error: fail-closed",
