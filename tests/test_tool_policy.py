...
class TestStrictPolicy:
    def test_allowed_tool(self, engine):
        ...
        result = engine.evaluate_tool_call(tc, "acme", "support-bot")
    ...
    def test_rate_limit_exceeded(self, engine):
        ...
        result = engine.evaluate_tool_call(tc, "acme", "support-bot", call_count=5)
        assert result.verdict == Verdict.BLOCK
...
class TestBatchEvaluation:
    def test_batch_blocks_on_first_violation(self, engine):
        ...
        result = engine.evaluate_tool_calls(calls, "acme", "support-bot")
        assert result.verdict == Verdict.BLOCK
