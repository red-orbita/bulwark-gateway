...
| `BULWARK_MCP_SCANNING_ENABLED` | bool | `false` | Opt-in: register the `McpToolScanner` (INPUT_ASYNC, GA). Scans the request's `tools` array (MCP/OpenAI tool definitions) with the shared stdlib MCP tool-poisoning core (`BWK-MCP-TP1..TP4` — hidden instructions, unicode deception, param injection); the least-privilege core is admin/config-time only (needs source, N/A to a runtime tools array). Deterministic, measured at 100% detection / 0 FP over the TP1..TP4 corpus; inert/not registered when off |
...
| `BULWARK_TRIFECTA_RUNTIME_ENABLED` | bool | `false` | Opt-in: register the runtime lethal-trifecta accumulator (`src/correlation/trifecta_runtime.py`). Complements the config-time trifecta analyzer with *behaviour*: accumulates, per origin (authenticated `subject` when known, else `session`=tenant+agent) over a sliding window, which of the three breach-enabling pillars the origin has actually exercised — DATA_ACCESS (tools invoked → capabilities via the shared `pillars_for_tools` SSOT), UNTRUSTED_EXPOSURE (a suspicious INPUT this request), EXFILTRATION (a sensitive OUTPUT this request). On the request that *first* completes all three (the `newly_completed` transition, so it fires once not every request) it emits an `EXCESSIVE_AGENCY` `SecurityEvent` (`source=trifecta_runtime`, `metadata.correlation=True` so the event tap skips it) and elevates the origin's risk state (subject/session/tenant bumps) so adaptive enforcement hardens the next requests. Fully inert (no state, no I/O) when off; Redis-backed with in-memory fallback + circuit breaker; fail-open (never breaks the response path) |
| `BULWARK_TRIFECTA_RUNTIME_BLOCKING` | bool | `false` | When on, the request that completes the trifecta is BLOCKed (content scrubbed, tool_calls stripped, verdict `critical`); otherwise it only WARNs + records the event and elevates risk (observe-first rollout) |
| `BULWARK_TRIFECTA_RUNTIME_WINDOW_SECONDS` | float | `1800.0` | Sliding window (seconds) over which pillars accumulate for an origin. A pillar not re-exercised within the window decays out, so the trifecta reflects *recent* behaviour rather than lifetime activity |
| `BULWARK_INVESTIGATION_ENRICH_AUTO_RAISE` | bool | `false` | Human gate for the investigation center's enrichment→enforcement coupling. When on, a **malicious** Cortex/OpenCTI/MISP verdict on a case observable auto-hardens every `origin` linked to the case to the correlation block threshold. Off by default: the observable is still flagged `is_ioc`, but no origin is hardened — the enrich/lookup response surfaces `origin_risk.eligible_origins` + `skipped_reason="auto_raise_disabled"` so an operator hardens them deliberately (a single false-positive analyzer verdict never silently blocks a tenant) |
| `BULWARK_INVESTIGATION_SESSION_TOOL_RPM` | int | `60` | **Admin-side**. Per-operator sliding-window budget (requests/minute) for the investigation observable enrich/lookup/respond endpoints, each of which reaches an external threat-intel platform (Cortex/OpenCTI/MISP) per call. Caps **operator sessions/JWTs** only — service-account keys are already throttled per-key inside `require_permission_automation` and pass through untouched (no double-count). Over-budget ⇒ `429` + `Retry-After: 60` + a `investigation.tool_call_rate_limited` audit record. Reuses the automation limiter (Redis-first, in-memory fallback) under a distinct `investigation-tool:` key namespace; `<= 0` disables the operator cap |
...
| POST | `/admin/investigation/cases/{id}/observables/{obs}/enrich` | Session | Enrich an observable via a Cortex integration's analyzers (folds worst-level verdict into `enrichment['cortex']`, flags `is_ioc` on malicious; a malicious verdict may auto-raise origin-risk on every `origin` subject linked to the case — **human-gated, off by default** via `BULWARK_INVESTIGATION_ENRICH_AUTO_RAISE`; when off surfaces `origin_risk.eligible_origins` + `skipped_reason="auto_raise_disabled"` instead, best-effort/fail-open, returns `origin_risk.raised`; fail-open 502) — `investigation:write` |
...
| GET | `/admin/integrations/{id}/collections` | Session | List a TAXII integration's collections (id/title/`can_read`/`can_write`) so an operator can pick a writable one before wiring a push (taxii-only; fail-open 502) — `integrations:read` |
| POST | `/admin/integrations/reload` | Session | Reload connector registry from disk — `integrations:write` |
| POST | `/admin/integrations/push/case/{case_id}` | Session | Idempotent push of a case to TheHive/IRIS/OpenCTI/MISP/TAXII (create-or-update via link store; fail-open). OpenCTI materialises SCOs + indicators + sightings + a container report via raw GraphQL, MISP materialises an Event + Attributes, TAXII publishes a STIX 2.1 bundle (report + SCOs + indicators) into a writable collection via the "Add Objects" operation, all gated by a TLP data-sharing policy — `TLP:RED` observables are excluded and a wholly-restricted case is refused with `400` (never contacts the remote for restricted data) — `integrations:write` |
...
| POST | `/admin/integrations/sync/case/{case_id}/resolve-conflict` | Session | Operator adjudication of a reconcile `conflict` (a remote reopen of a locally-terminal case). Body `resolution`=`accept_remote` (re-run with the anti-reopen guard disarmed → the case reopens → `synced`) or `keep_local` (workflow untouched; an audited `[remote]`-tagged note records the choice; link marked `synced`). Session-only — a human decision, not an automation verb — `integrations:write` |
...
| GET | `/admin/integrations/webhooks/status` | Session | Delivery-observability snapshot for the fire-and-forget fan-out: process-local `total_delivered`/`total_failed` counters + `last_error` + each current subscription's last-delivery outcome (ok/detail/event/at). In-memory (single-replica admin), recorded at the single delivery chokepoint so test pings and lifecycle deliveries alike are counted; no secret exposed — `integrations:read` |
