...
| **F2** | 🟠 | Hot-path Redis cost unbounded by faults. `evaluate_origin_risk` = 2 RTTs (session+tenant get) every non-blocked request; a confirmed incident adds ~6 more (3 bumps × read+write). `socket_timeout=1s`, **no circuit breaker** ⇒ Redis latency spikes hit user latency ×N. | `incident.py:322-323`; `risk_state.py:94,163-185` | R2: pipeline reads (1 RTT) + circuit breaker degrading to in-memory | _pending_ | OPEN |
