...
> **Status:** COMPLETE — F1–F5 all CLOSED, A1–A8 signed off (§4). Post-sign-off
> enhancement **E6** (bulk-PII block-recall) also CLOSED without regressing any
> criterion.
...
| _tbd_ | E6 (enh) | PII-multiplicity signal in `confidence.py` (distinct SSN/email/phone/card ≥3 ⇒ +0.45, value-deduped, anchored no-ReDoS); +2 boundary benign in corpus; render label corrected; **`.gitignore` fix** — the R5 corpus (`tests/data/correlation_corpus.py`) was silently matched by the broad `data/` rule and never committed in `8f01b27`, so `test_correlation_fp_fn.py` would `ModuleNotFoundError` on a fresh clone; now un-ignored and tracked | E6 → CLOSED | A6 ✅ **strengthened** (pii block-recall 0.00→0.778, target ≥0.70 now asserted) + **now genuinely evidence-closed** (corpus committed, not just its test), A7 ✅ (unchanged when disabled), A8 ✅ (6+3 tests, ruff+mypy clean on touched modules) | 1610 passed / 4 skipped full suite (was 1602) — **0 regressions**. FP invariant preserved: **0 FP** on 22 benign (max 0.48 < 0.50). Honest residual kept: unstructured PII (addresses/DoB) still WARN + accrues risk, pinned by `test_unstructured_pii_leak_is_warn_tier_not_silently_passed`. Adversarial dedup pinned (padding one SSN 50× cannot forge the bulk signal). No new deps. |
...
- Rewriting the confidence heuristic into an ML model. R5 (F5) quantified the
  heuristic and E6 raised bulk-PII recall: it is decisive on credential/secret
  leaks (block-recall 1.00, 0 FP) and now on *bulk structured* PII dumps
  (block-recall 0.778 via the multiplicity signal). Leaks of purely *unstructured*
  PII (street addresses, dates of birth) have no regex-recognisable identifier and
  remain WARN-tier — they still WARN and accrue risk (adaptive escalation), and
  the corpus/harness measure this residual so any future ML replacement is held to
  the same bar. Closing that last gap is an explicit non-goal here.
