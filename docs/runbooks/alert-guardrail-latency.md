# Runbook: BulwarkGuardrailLatencyHigh

## Alert Details

- **Severity**: Critical
- **Alert rule**: `BulwarkGuardrailLatencyHigh`
...
2. **Confirm latency is actually elevated** (not a metric artifact):
   ...
   # histogram_quantile(0.99, sum(rate(bulwark_request_duration_seconds_bucket{phase="input_guardrail"}[5m])) by (le))
...
3. **Check CPU pressure** (regex processing is CPU-bound):
   ```bash
   kubectl top pods -l app.kubernetes.io/name=bulwark-proxy -n bulwark-gateway
   ```
...
# Prometheus:
# histogram_quantile(0.99, sum(rate(bulwark_request_duration_seconds_bucket{phase="input_guardrail"}[5m])) by (le))
# histogram_quantile(0.99, sum(rate(bulwark_request_duration_seconds_bucket{phase="total"}[5m])) by (le))
...
# Prometheus: sum(rate(bulwark_requests_total[5m]))

# 6. Check if HPA is scaling (are we at max replicas?)
kubectl get hpa -n bulwark-gateway
kubectl describe hpa proxy -n bulwark-gateway

# 7. Pod resource limits
kubectl get deploy/proxy -n bulwark-gateway -o jsonpath='{.spec.template.spec.containers[0].resources}'
...
kubectl scale deploy/proxy --replicas=5 -n bulwark-gateway

# 2. Verify HPA max allows this
kubectl get hpa proxy -n bulwark-gateway -o jsonpath='{.spec.maxReplicas}'

# 3. If at max, increase HPA max
kubectl patch hpa proxy -n bulwark-gateway --type=json -p='[
  {"op":"replace","path":"/spec/maxReplicas","value":10}
]'

# 4. Increase CPU request/limit for better scheduling
kubectl patch deploy proxy -n bulwark-gateway --type=json -p='[
...
# Watch: histogram_quantile(0.99, sum(rate(bulwark_request_duration_seconds_bucket{phase="input_guardrail"}[2m])) by (le))
...
kubectl set env deploy/proxy BULWARK_MAX_MESSAGE_LENGTH=32768 -n bulwark-gateway
...
```bash
# 1. Check current QPS
# Prometheus: sum(rate(bulwark_requests_total[1m]))

# 2. If from specific tenant, apply rate limit
kubectl exec deploy/redis -n bulwark-gateway -- \
  redis-cli SET bulwark:rate_limit:override:<TENANT_ID> 10

# 3. Scale up to handle load
kubectl scale deploy/proxy --replicas=8 -n bulwark-gateway
```
...
- [`BulwarkSLOLatencyBreach`](alert-guardrail-latency.md) — guardrail latency directly contributes to SLO
- [`BulwarkMemoryPressure`](alert-guardrail-latency.md) — GC pauses can cause latency spikes
- [`BulwarkBackendErrorRateHigh`](alert-backend-errors.md) — backend slowness may compound latency
- [`BulwarkHighBlockRate`](alert-high-block-rate.md) — attack traffic can increase guardrail processing load
- [`BulwarkMLScannerSlow`](alert-guardrail-latency.md) — ML phase adds to total latency
