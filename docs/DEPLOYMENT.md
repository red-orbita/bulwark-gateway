...
2. **Enable the capability flag** (boot-time env, default off):
   ...
   > **Multilingual footprint note.** Accurate language detection needs a real
   > backend; two opt-in options exist (neither is in core, so the default image
   > stays minimal):
   > - **lingua** — `pip install bulwark-gateway[multilingual]`, a **~170 MB**
