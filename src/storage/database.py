...
def _read_db_url() -> str:
    ...
    if url_file is not None:
        try:
            with open(url_file, encoding="utf-8") as secret_file:
                value = secret_file.read(16385).strip()
            if not value or len(value) > 16384:
                raise ValueError("Invalid database URL secret")
            return value
        except (OSError, UnicodeError, ValueError):
            raise RuntimeError("Configured database URL secret must be readable and nonempty") from None
...
class QueryTranslator:
    ...
    def translate(self, query: str, params: Optional[Sequence] = None) -> tuple[str, Optional[Sequence]]:
        ...
        # Only the driver's resolved PostgreSQL type can decide how to encode a
        # parameter. An ISO-looking tenant/title is still TEXT, not a timestamp.
...
class SQLiteEngine(DatabaseEngine):
    ...
    async def init(self) -> None:
        ...
        # Concurrent first opens may race while promoting a new DB to WAL;
        # journal_mode can report BUSY immediately despite busy_timeout.
...
async def _init_pg_codecs(conn: Any) -> None:
    """Bind ISO timestamps by PostgreSQL type, never by parameter appearance.

    Tuple codecs avoid DateStyle/session timezone dependencies of text codecs.
    They also retain asyncpg's binary support for arrays and COPY operations.
    """
    for name, aware in (("timestamp", False), ("timestamptz", True)):
        epoch = datetime(2000, 1, 1, tzinfo=timezone.utc if aware else None)

        def encode(value: Any, *, aware: bool = aware, epoch: datetime = epoch) -> tuple[int]:
            try:
                if isinstance(value, str):
                    # Bound parsing and reject precision loss, date-only input,
                    # relative dates and PostgreSQL's session-dependent literals.
                    if len(value) > 48 or not re.fullmatch(
                ...
                if not isinstance(value, datetime) or (value.utcoffset() is not None) != aware:
                    raise ValueError
                if aware:
                ...
                raise ValueError(
                    "Expected an offset-aware ISO datetime for timestamptz" if aware
                    else "Expected a naive ISO datetime for timestamp"
                ) from None

        def decode(value: tuple[int], *, epoch: datetime = epoch) -> datetime:
            # Match the driver's datetime representation of PostgreSQL infinity.
            if value[0] == 2**63 - 1:
                return datetime.max
            if value[0] == -(2**63):
                return datetime.min
            return epoch + timedelta(microseconds=value[0])

        try:
            await conn.set_type_codec(name, schema="pg_catalog", encoder=encode, decoder=decode, format="tuple")
        except Exception:
            raise RuntimeError("Failed to configure PostgreSQL timestamp codecs") from None
...
def _pg_row(record: Any) -> Row:
    ...
    Type-directed codecs handle the *write* side (ISO string or datetime);
    this is the symmetric *read* side: datetime/date → ISO string,
...
class PostgreSQLEngine(DatabaseEngine):
    ...
    async def init(self) -> None:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                logger.warning(
                    "PostgreSQL connection failed (attempt %d/%d). "
                    "Retrying in %.1fs...",
    ...
    def _run_sync(self, coro):
        ...
        # Do not mistake a connection/codec RuntimeError for "no running loop"
        # and retry an already-awaited coroutine, hiding the original failure.
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            return pool.submit(asyncio.run, coro).result(timeout=10)

    def _get_connect_kwargs(self) -> dict[str, Any]:
        """Apply the same TLS policy to pooled and direct connections."""
        if not self._ssl:
            # Preserve asyncpg's DSN/environment TLS settings when not overridden.
            return {}
        if self._ssl_mode == "disable":
            return {"ssl": False}
        if self._ssl_mode not in ("verify-full", "verify-ca", "require"):
            raise ValueError("Unsupported PostgreSQL TLS mode") from None

        import ssl as ssl_module

        try:
            ssl_ctx = ssl_module.create_default_context()
            ssl_ctx.minimum_version = ssl_module.TLSVersion.TLSv1_2
            if self._ssl_mode != "verify-full":
                ssl_ctx.check_hostname = False
            if self._ssl_mode == "require":
                ssl_ctx.verify_mode = ssl_module.CERT_NONE
        except Exception:
            raise RuntimeError("Failed to configure PostgreSQL TLS") from None
        return {"ssl": ssl_ctx}
...
def create_engine(url: Optional[str] = None) -> DatabaseEngine:
    ...
    if db_url.startswith("sqlite"):
        ...
        raise ValueError(
            "Unsupported database URL scheme. Supported: sqlite, postgresql"
        ) from None
