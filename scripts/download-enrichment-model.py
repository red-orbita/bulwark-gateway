...
TARGET_DIR = Path(os.getenv(
    "SENTENCE_TRANSFORMERS_HOME",
    sys.argv[2] if len(sys.argv) > 2 else "/app/shared/enrichment/sentence-transformers",
))
...
def download_file(url: str, dest: Path, expected_size: int | None = None) -> bool:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "bulwark-gateway/0.2.0"})  # noqa: S310 - trusted hardcoded HuggingFace HTTPS URL, not user input
        with urllib.request.urlopen(req, timeout=120) as resp:  # noqa: S310 - trusted hardcoded HuggingFace HTTPS URL, not user input
...
def get_model_files(model_name: str) -> list[dict]:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "bulwark-gateway/0.2.0"})  # noqa: S310 - trusted hardcoded HuggingFace HTTPS URL, not user input
        with urllib.request.urlopen(req, timeout=30) as resp:  # noqa: S310 - trusted hardcoded HuggingFace HTTPS URL, not user input
...
def main():
    print("=== Bulwark Gateway Model Download ===")
